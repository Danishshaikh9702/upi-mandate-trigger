...
INVOICE_SCHEDULED_ON = os.environ["INVOICE_SCHEDULED_ON"].strip()[:10]
excel_file = excel_file_from_env()


def find_missed_upi_invoices():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            """
            cursor.execute(query, (INVOICE_SCHEDULED_ON,))
            results = cursor.fetchall()
            done_customers = set(str(row["customer_id"]) for row in results)
            print(f"Found {len(done_customers)} customers who have completed NACH today.")
    finally:
        connection.close()
    ...
    print(f"Filtered missed UPI invoice customers: {len(missed_df)} out of {len(df)}")

    output_dir = "missed_upi_invoices"
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    os.makedirs(output_dir, exist_ok=True)

    missed_df.drop(columns=["customer_id_str"], inplace=True)
    missed_df.to_excel(os.path.join(output_dir, "missed_upi_invoices.xlsx"), index=False)
